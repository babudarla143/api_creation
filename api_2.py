...
def create_db():
    """Create a database and API keys table if not exists."""
...
def insert_api_key(username, api_key):
    ...
    with sqlite3.connect(DB_FILE) as conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO api_keys (username, api_key) VALUES (?, ?)", (username, api_key))
        conn.commit()
...
def api_creation():
    ...
    try:
        insert_api_key(username, api_key)
        return jsonify({"message": "API key created", "username": username, "api_key": api_key}), 200
...
def predict():
    ...
    if not api_key or not get_api_key(api_key):
